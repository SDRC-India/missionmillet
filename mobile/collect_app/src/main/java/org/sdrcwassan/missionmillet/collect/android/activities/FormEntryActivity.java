...
import org.sdrcwassan.missionmillet.collect.android.dao.FormsDao;
import org.sdrcwassan.missionmillet.collect.android.dao.InstancesDao;
import org.sdrcwassan.missionmillet.collect.android.exception.GDriveConnectionException;
...
import org.sdrcwassan.missionmillet.collect.android.listeners.FormSavedListener;
import org.sdrcwassan.missionmillet.collect.android.listeners.SavePointListener;
import org.sdrcwassan.missionmillet.collect.android.logic.FormController;
import org.sdrcwassan.missionmillet.collect.android.logic.FormController.FailedConstraint;
import org.sdrcwassan.missionmillet.collect.android.utilities.TimerLogger;
import org.sdrcwassan.missionmillet.collect.android.preferences.AdminKeys;
import org.sdrcwassan.missionmillet.collect.android.preferences.AdminPreferencesActivity;
...
import org.sdrcwassan.missionmillet.collect.android.provider.FormsProviderAPI.FormsColumns;
import org.sdrcwassan.missionmillet.collect.android.provider.InstanceProviderAPI;
import org.sdrcwassan.missionmillet.collect.android.provider.InstanceProviderAPI.InstanceColumns;
import org.sdrcwassan.missionmillet.collect.android.tasks.FormLoaderTask;
import org.sdrcwassan.missionmillet.collect.android.tasks.SavePointTask;
import org.sdrcwassan.missionmillet.collect.android.tasks.SaveResult;
import org.sdrcwassan.missionmillet.collect.android.tasks.SaveToDiskTask;
...
import org.sdrcwassan.missionmillet.collect.android.views.ODKView;
import org.sdrcwassan.missionmillet.collect.android.widgets.QuestionWidget;
import org.sdrcwassan.missionmillet.collect.android.widgets.StringWidget;
...
public class FormEntryActivity extends AppCompatActivity implements AnimationListener,
                        ...
                        .setOnClickListener(new OnClickListener() {
                            ...
                            public void onClick(View v) {
                                ...
                                if (saveAs.getText().length() < 1) {
                                    ...
                                    if(checkFileSize()){
                                       // Toast.makeText(FormEntryActivity.this,"File size exceeds 5 mb",Toast.LENGTH_LONG).show();
                                        createErrorDialog(getResources().getString(R.string.file_size_exceeds),true);
                                    }else{
    ...
    private boolean checkFileSize() {
        ...
        String instanceFolder1 = Collect.getInstance().getFormController().getInstancePath()
                .getParent();
        if(instanceFolder1!=null){
            File file = new File(instanceFolder1);
            if(file!=null){
                File[] listFile = file.listFiles();
                if(listFile.length>0){
                    Log.v("","");
                    for(int i=0;i<listFile.length;i++){
                        File imageFile = new File(String.valueOf(listFile[i]));
                        filesize = filesize+getFileSizeMegaBytes(imageFile);

                    }
                    if((int)filesize>5){isValid = true;
                    }
                }
...
/*
    ...
    public void loadingComplete(FormLoaderTask task) {
        ...
        // things then
        // returned to DGA Collect and chose Edit Saved Form, but that the
        // savepoint for that
    ...
    private void sendSavedBroadcast() {
        Intent i = new Intent();
        i.setAction("org.sdrcwassan.missionmillet.collect.android.FormSaved");
