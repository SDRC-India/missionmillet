...
import org.sdrcwassan.missionmillet.collect.android.external.ExternalDataUtil;
import org.sdrcwassan.missionmillet.collect.android.listeners.AdvanceToNextListener;
import org.sdrcwassan.missionmillet.collect.android.listeners.AudioPlayListener;
...
public class SelectOneSearchWidget extends QuestionWidget implements
        OnCheckedChangeListener, AudioPlayListener, View.OnClickListener  {

    List<SelectChoice> items; // may take a while to compute
    ArrayList<RadioButton> buttons;
    protected EditText searchStr;
    /**
     * autoAdvanceToNext is used to check whether the apperance is quicksearch or not.
    ...
    public SelectOneSearchWidget(Context context, FormEntryPrompt prompt) {
        ...
        /**
         * if the apperance is quicksearch then autoAdvanceToNext is set to true..
         * @author Subhadarshani
         */
        if(prompt.getAppearanceHint().contains("quick")){
    ...
    public void createOptions(List<SelectChoice> searchedItems, List<Integer> tagList) {
        ...
        if (searchedItems != null && searchedItems.size() > 0) {
            for (int i = 0; i < searchedItems.size(); i++) {
                ...
                /**
                 * if the apperance is quicksearch then the quick functionality is added.
                 * @author Subhadarshani
                 */
                if(autoAdvanceToNext){
                    ...
                    rl.addView(rightArrow, lp);

                    mediaLayout.addView(rl);
                }
